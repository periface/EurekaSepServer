...
using Eureka.Spe.Migrations.SeedData;
...
namespace Eureka.Spe.Tests
{
    public abstract class SpeTestBase : AbpIntegratedTestBase<SpeTestModule>
    {
        ...
        protected void UsingDbContext(Action<SpeDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SpeDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SpeDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SpeDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SpeDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SpeDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SpeDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SpeDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SpeDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SpeDbContext, Task<T>> func)
        {
            T result;
